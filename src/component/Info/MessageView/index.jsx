...
class MessageView extends Component {
  ...
  renderInfo() {
    ...
    return (
      <div
        className={"wasabi-message " + this.props.type}
        style={{ display: this.state.visible ? "block" : "none" }}
      >
        <div className="notice">
          <i className={iconCls} style={{ marginRight: 10, fontSize: 16 }}></i>
          {this.props.msg}
        </div>
      </div>
    );
  ...
  renderConfirm() {
    return (
      <div
        className={
          " wasabi-overlay " + (this.state.visible == true ? "active " : "")
        }
      >
        <div
          className={
            "wasabi-confirm " +
            (this.state.visible ? " wasabi-scale-in" : " wasabi-scale-out")
          }
          style={{ display: this.state.visible ? "block" : "none" }}
        >
          <div className="wasabi-confirm-title">
            <i className="icon-question" style={{ marginRight: 5 }}></i>提示
          </div>
          ...
          {this.state.visible ? (
            <div className="buttons">
              ...
              <button type="button" className="ok" onClick={this.OKHandler}>
                确定
              </button>
            </div>
          ) : null}
...
MessageView.propTypes = {
  type: PropTypes.oneOf(["loading", "alert", "success", "error", "confirm"]),
