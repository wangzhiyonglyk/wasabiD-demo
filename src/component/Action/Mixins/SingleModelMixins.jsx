...
let PageModelMixins = {
  ...
  submitButton: function(disabled) {
    //提交按钮对象
  ...
  initButtons: function() {
    //
    let add = new ButtonModel('add', '新增', 'green');
    add.onClick = this.addOpen;
    return [
      add,
      new ButtonModel('update', '更新'),
      new ButtonModel('delete', '删除')
    ];
  },
  initModel: function() {
    //获取数据模型
    let fetchModel = new FetchModel(
      this.state.modelUrl,
      this.initModelSuccess,
      null,
      this.fetchErrorHandler
    );
    unit.fetch.get(fetchModel);
  },
  initModelSuccess: function(result) {
    //获取数据模型成功
    ...
    if (result.data != null && result.data instanceof Array) {
      ...
      for (let index = 0; index < result.data.length; index++) {
        var modelOject = new FormModel(
          result.data[index].name,
          result.data[index].label
        );
        modelOject = Object.assign(modelOject, result.data[index]); //合并属性
        modelOject.filterAble =
          modelOject.filterAble == undefined ? true : modelOject.filterAble;
        modelOject.gridAble =
          modelOject.gridAble == undefined ? true : modelOject.gridAble;
