...
let Import = React.createClass({
  ...
  onChange: function(event) {
    ...
    if (files.length > 0) {
      if (files[0].type == '') {
        //苹果电脑存在的现象
        if (
          (files[0].name.lastIndexOf('.xls') > -1 &&
            files[0].name.lastIndexOf('.xls') + 3 ==
              files[0].name.length - 1) ||
          (files[0].name.lastIndexOf('.xlsx') > -1 &&
            files[0].name.lastIndexOf('.xlsx') + 3 == files[0].name.length - 1)
        ) {
          typevalidate = true;
  ...
  importBegin: function(name, title) {
    //开始的导入
  ...
  giveup: function() {
    ...
    uploadInfo.unshift(
      <div className='info' key={'success' + (index + 2).toString()}>
        {'用户终止,成功数:' +
          this.successNum.toString() +
          ',失败数:' +
          this.failNum.toString()}
      </div>
    );
    this.setState({
  ...
  importHandler: function(index) {
    ...
    xhr.upload.addEventListener(
      'progress',
      this.uploadProgress.bind(this, index),
      false
    ); //上传进度
      ...
      if (index == null) {
        ...
        if (percentComplete < 100) {
          this.setState({
            uploadInfo: [
              <div
                key={'upload' + percentComplete.toString()}
                className='success'
              >
      ...
      if (result && result.success != null && result.success != undefined) {
        if (result.success == true) {
          if (index == null) {
            ...
            uploadInfo.unshift(
              <div key='read' className='success'>
            ...
            if (result.data.success) {
              ...
              this.failNum += 1;
              uploadInfo.unshift(
                <div className='fail' key={'fail' + (index + 2).toString()}>
                  {'序号为' +
                    (index + 2).toString() +
                    '行,导入失败,' +
                    result.data.message}
          ...
          if (index == null) {
            ...
            if (index >= this.total - 1) {
              //代表已经执行完最后一条记录了
              uploadInfo.unshift(
                <div className='info' key={'successall'}>
                  {'所有数据执行完成,成功数:' +
                    this.successNum.toString() +
                    ',失败数:' +
                    this.failNum.toString()}
      ...
      <Modal ref='modal' width={460} height={340} title='请选择导入文件'>
        <div className='import-section'>
          ...
          <input
            ...
            {...props}
            style={{ display: this.state.choseDisabled ? 'none' : 'inline' }}
          ></input>
          <Button
            type='button'
            disabled={this.state.choseDisabled}
        ...
        <div className='import-submit'>
          <a
            className='import-failload'
            target='blank'
            href={this.props.failloadurl}
            style={{ display: this.state.showfail == true ? 'inline' : 'none' }}
          >
            下载失败信息
          </a>
          <Button
            ...
            onClick={this.importBegin}
          ></Button>
          <Button
            title='终止'
            disabled={this.state.giveupdisabled}
            onClick={this.giveup}
            theme='cancel'
          ></Button>
          <Button title='关闭' onClick={this.close} theme='cancel'></Button>
        </div>
        <a className='import-downloadmodel' href={this.props.modelurl}>
          下载模版
        </a>
