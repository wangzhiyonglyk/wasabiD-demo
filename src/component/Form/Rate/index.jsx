...
class Rate extends Component {
      ...
      while (index <= num) {
        try {
          ...
          let iconCls =
            index * percent <= value
              ? "icon-star"
              : index * percent > value && (index - 1) * percent < value
              ? "icon-star-half"
              : "icon-star-blank";
          ...
          control.push(
            <i
              key={index}
              style={{
                fontSize: this.props.starSize,
                color: iconCls == "icon-star-blank" ? null : color,
              }}
              className={"wasabi-rate-star " + iconCls}
              onClick={this.onSelect.bind(this, index * percent)}
              onMouseOver={this.onMouseOver.bind(this, index * percent)}
            ></i>
          );
        } catch (e) {
  ...
  render() {
    return (
      <div
        className={"wasabi-rate " + (this.props.className || "")}
        style={this.props.style}
        onMouseOut={this.onMouseOut.bind(this)}
      >
        {this.renderStar()}
        <a className="wasabi-rate-text" style={{ color: this.props.textColor }}>
          {this.props.readOnly || this.props.disabled || this.props.showValue
            ? this.state.value
            : this.renderText()}
        </a>
      </div>
