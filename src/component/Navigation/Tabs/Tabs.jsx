  ...
  componentDidUpdate() {
    try {
      ...
      if (dot && dot.children && dot.children.length > 0) {
        if (
          dot.children[this.state.activeIndex].offsetLeft - 40 >
          dot.clientWidth
        ) {
          dot.scrollLeft =
            dot.children[this.state.activeIndex].offsetLeft -
            40 -
            dot.clientWidth +
            dot.children[this.state.activeIndex].clientWidth;
        }
      }
    } catch (e) {}
  }
        ...
        <div className={"wasabi-tab-nav " + (this.props.plain ? "plain" : "")}>
          {this.props.plain ? null : (
            ...
            >
              ...
              <LinkButton
                ...
                iconCls="icon-angle-double-left"
              ></LinkButton>
            </div>
          )}
          <div
          ...
          >
            {React.Children.map(this.props.children, (child, index) => {
              if (child) {
                let iconCls =
                  child && child.props.iconCls ? child.props.iconCls : "txt";
                return (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    {child.props.title}
                    <i
                      className="tab-icon icon-close"
                      style={{
                        display: child.props.closeAble ? "inline" : "none",
                      }}
          ...
          {this.props.plain ? null : (
            <div
              ...
              title="下一个"
              className={"wasabi-tab "}
              style={{ float: "left", textAlign: "center", width: 40 }}
            >
              {" "}
              <LinkButton
                style={{ transform: "translateY(-3px)" }}
          ...
          {this.props.plain ? null : (
            <div
              ...
              title="刷新"
              className={"wasabi-tab "}
              style={{ float: "right", textAlign: "center", width: 40 }}
            >
              {" "}
              <LinkButton
                style={{ transform: "translateY(-3px)" }}
