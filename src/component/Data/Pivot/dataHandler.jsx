...
export default {
      ...
      /**继续处理列纬度的表头 */
      {
        ...
        for (let col = 0; col < columns.length; col++) {
          for (let i = 0; i < headers[col].length; i++) {
            let colSpan = 0;
            for (let j = 0; j < headers[columns.length].length; j++) {
              //通过最后一行数据行来判断
              if (
                headers[columns.length][j].name.indexOf(headers[col][i].name) >
                -1
              ) {
                colSpan++;
              }
  ...
  setHeaders(columns = [], values = [], headers = [], dataCurrentRow = {}) {
    ...
    for (let col = 0; col < columns.length; col++) {
      colnamestr.push(
        columns[col].name + "|" + dataCurrentRow[columns[col].name] || ""
      ); //保存前面加当前的，用于最终数据列取值的对应的字段name
      //处理列
      if (
        headers[col].filter((item) => {
          return item.name == colnamestr.join("-");
        }) == 0
      ) {
        //没有添加此列
        headers[col].push({
    ...
    for (let v = 0; v < values.length; v++) {
      if (
        headers[columns.length].filter((item) => {
          return item.name == colnamestr.join("-") + "-" + values[v].name;
        }) == 0
      ) {
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
      ...
      rowidstr = rowidstr
        ? rowidstr +
          ...
          rows[rowIndex].name +
          "|" +
          dataCurrentRow[rows[rowIndex].name]
        : rows[rowIndex].name + "|" + dataCurrentRow[rows[rowIndex].name];
      if (
        rowsTreeData.filter((item) => {
          return item.id == rowidstr;
        }).length == 0
      ) {
        rowsTreeData.push({
          id: rowidstr,
