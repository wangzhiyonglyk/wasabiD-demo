...
class Transfer extends React.Component {
  constructor(props) {
    ...
    let realData = propsTran.preprocessForm(
      this.props.data,
      this.props.valueField,
      null,
      this.props.textField
    );
    let realSelectData = propsTran.preprocessForm(
  ...
  static getDerivedStateFromProps(props, state) {
    let newState = {};
    if (props.url && props.params && func.diff(props.params, state.params)) {
      //如果有url
      newState = {
    ...
    if (
      ...
      func.diff(props.data, state.data)
    ) {
      //如果传了固定数据
      newState.data = propsTran.setComboxValueAndText(
        "transfer",
        "",
        props.data,
        state.valueField,
      ...
      newState.selectData = propsTran.setComboxValueAndText(
        "transfer",
        "",
        props.selectData,
        state.valueField,
  ...
  itemDblClickHandler(direction, indexArray) {
    if (indexArray == null) {
      ...
      this.setState(
        ...
        () => {
          this.props.onSelect &&
            this.props.onSelect(this.state.selectData, this.state.data);
        }
      );
    }
  }
  itemUpHandler() {
    ...
    if (this.state.leftOnIndex && !(this.state.leftOnIndex instanceof Array)) {
      ...
      this.state.data.splice(
        this.state.leftOnIndex - 1,
        2,
        this.state.data[this.state.leftOnIndex],
        this.state.data[this.state.leftOnIndex - 1]
      );
      this.setState(
        {
          data: this.state.data,
      ...
      this.state.rightOnIndex &&
      !(this.state.rightOnIndex instanceof Array)
    ) {
      //右边
      this.state.selectData.splice(
        this.state.rightOnIndex - 1,
        2,
        this.state.selectData[this.state.rightOnIndex],
        this.state.selectData[this.state.rightOnIndex - 1]
      );
  ...
  itemDownHandler() {
    //向下
    if (
      this.state.leftOnIndex !== null &&
      !(this.state.leftOnIndex instanceof Array) &&
      this.state.leftOnIndex < this.state.data.length - 1
    ) {
      //左边

      this.state.data.splice(
        this.state.leftOnIndex,
        2,
        this.state.data[this.state.leftOnIndex + 1],
        this.state.data[this.state.leftOnIndex]
      );
      ...
      this.state.rightOnIndex !== null &&
      !(this.state.rightOnIndex instanceof Array) &&
      this.state.rightOnIndex < this.state.selectData.length - 1
    ) {
      //右边
      this.state.selectData.splice(
        this.state.rightOnIndex,
  ...
  render() {
    ...
    this.state.data.map((item, index) => {
      leftControl.push(
        <li
          className={
            this.state.leftOnIndex == index ||
            (this.state.leftOnIndex instanceof Array &&
              this.state.leftOnIndex.indexOf(index) > -1)
              ? "on"
              : ""
          }
          key={index}
          onDoubleClick={this.itemDblClickHandler.bind(this, "right", index)}
          onClick={this.itemClickHandler.bind(this, "left", index)}
        >
          {item.text}
        </li>
      );
    ...
    this.state.selectData.map((item, index) => {
      rightControl.push(
        <li
          className={
            this.state.rightOnIndex == index ||
            (this.state.rightOnIndex instanceof Array &&
              this.state.rightOnIndex.indexOf(index) > -1)
              ? "on"
              : ""
          }
          key={index}
          onDoubleClick={this.itemDblClickHandler.bind(this, "left", index)}
          onClick={this.itemClickHandler.bind(this, "right", index)}
        >
          {item.text}
        </li>
      );
    ...
    return (
      ...
      >
        <ul className="wasabi-transfer-left">{leftControl}</ul>
        <div className="wasabi-transfer-middle">
          <LinkButton
            name="up"
            title="向上"
            iconCls={"icon-up"}
            onClick={this.itemUpHandler}
            ref={this.up}
            disabled={
              this.state.leftOnIndex instanceof Array ||
              this.state.rightOnIndex instanceof Array
          ...
          <LinkButton
            ...
            title="向右"
            iconCls={"icon-right"}
            onClick={this.itemDblClickHandler.bind(
              this,
              "right",
              this.state.leftOnIndex
            )}
        ...
        <ul className="wasabi-transfer-right">{rightControl}</ul>
      </div>
    );
  }
}
