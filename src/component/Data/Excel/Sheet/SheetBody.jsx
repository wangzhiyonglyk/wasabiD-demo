...
class SheetBody extends React.Component {
  ...
  render() {
    ...
    let trArr = [];
    this.props.cells &&
      this.props.cells.map((rowData, rowIndex) => {
        let tds = [];
        rowData &&
          rowData.length > 0 &&
          rowData.map((cell, columnIndex) => {
            if (cell.hide !== true) {
              let className =
                cell.wrap === true ? " excel-td " : "excel-td ellipsis ";
              let style = {
              ...
              tds.push(
                <SheetCell
                  ...
                  onDoubleClick={this.props.onDoubleClick.bind(
                    this,
                    rowIndex,
                    columnIndex,
                    cell.label
                  ...
                  key={
                    "cell-" + rowIndex.toString() + "-" + columnIndex.toString()
                  }
                  className={className}
                  style={style}
                >
        ...
        trArr.push(
          <TableRow key={"row" + rowIndex.toString()}>
            <SheetCell className="wasabi-order-column">
              {rowIndex + 1}
            </SheetCell>
            {tds}
          </TableRow>
