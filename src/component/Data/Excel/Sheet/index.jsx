...
class Sheet extends React.Component {
  ...
  onChange(event) {
    ...
    if (
      this.state.startRowIndex !== null &&
      this.state.startColumnIndex !== null
    ) {
      //浅复制就行了
      sheet.cells[this.state.startRowIndex][this.state.startColumnIndex].label =
        event.target.value;
      sheet.cells[this.state.startRowIndex][this.state.startColumnIndex].value =
        event.target.value;
    }
    this.setState({
  ...
  onMouseDown(rowIndex, columnIndex, label, event) {
    ...
    if (event.button === 0) {
      ...
      this.startColumnIndex = event.shiftKey
        ? this.state.startColumnIndex
        : columnIndex;
  ...
  onMouseUp(rowIndex, columnIndex, rowSpan, colSpan, event) {
    if (this.startRowIndex !== null && this.startRowIndex !== undefined) {
      let selectIndexs = this.getRowColIndex(
        rowIndex,
        columnIndex,
        rowSpan,
        colSpan
      ...
      try {
        let begindom, enddom;
        begindom = document.getElementById(
          selectIndexs.startRowIndex + "-" + selectIndexs.startColumnIndex
        );
        enddom = document.getElementById(
          selectIndexs.endRowIndex + "-" + selectIndexs.endColumnIndex
        );
  ...
  onMouseMove(event) {
    if (this.startRowIndex !== null && this.startRowIndex !== undefined) {
      ...
      if (
        ...
        this.cancelChosed(); //取消选择
        for (
          let i = selectIndexs.startRowIndex;
          i <= selectIndexs.endRowIndex;
          i++
        ) {
          for (
            let j = selectIndexs.startColumnIndex;
            j <= selectIndexs.endColumnIndex;
            j++
          ) {
            try {
              if (
  ...
  dotHander(selectIndexs) {
    try {
      if (this.endRowIndex !== undefined && this.endRowIndex !== null) {
        //是点的拖动
        if (selectIndexs.startColumnIndex === selectIndexs.endColumnIndex) {
          ...
          if (
            ...
            //原来只有一个单元格，内容复制
            let sheet = this.dotCellCopyHandler(
              selectIndexs,
              this.state.sheet.cells[this.startRowIndex][this.startColumnIndex]
            );
            this.setState({
              sheet,
            });
            ...
            //递增
            let sheet = this.dotIncreaseHandler(
              selectIndexs,
              this.state.sheet.cells[this.endRowIndex][this.startColumnIndex]
            );
            this.setState({
              sheet,
            });
          ...
          if (
            ...
            let sheet = this.dotIncreaseHandler(
              selectIndexs,
              this.state.sheet.cells[this.startRowIndex][this.endColumnIndex]
            );
            this.setState({
              sheet,
            });
  ...
  dotIncreaseHandler(selectIndexs, cell) {
    ...
    try {
      //以数字作为结尾则递增，否则为复制
      let preValue = cell.value.match(/^\D+(.*\D)?/g)
        ? cell.value.match(/^\D+(.*\D)?/g)
        : "";
      let preLabel = cell.label.match(/^\D+(.*\D)?/g)
        ? cell.label.match(/^\D+(.*\D)?/g)[0]
        : "";
      let value = cell.value.match(/\d+(\.\d+)?$/g)
        ? cell.value.match(/\d+(\.\d+)?$/g)[0]
        : "";
      let label = cell.label.match(/\d+(\.\d+)?$/g)
        ? cell.label.match(/\d+(\.\d+)?$/g)[0]
        : "";
  ...
  getRowColIndex(rowIndex, columnIndex, rowSpan, colSpan) {
    let startRowIndex =
      this.startRowIndex > rowIndex ? rowIndex : this.startRowIndex;
    let endRowIndex =
      this.startRowIndex > rowIndex ? this.startRowIndex : rowIndex;
    let startColumnIndex =
      this.startColumnIndex > columnIndex ? columnIndex : this.startColumnIndex;
    let endColumnIndex =
      this.startColumnIndex > columnIndex ? this.startColumnIndex : columnIndex;
    //因为存在单元格合并的情况
    endRowIndex =
  ...
  onContextMenu(event) {
    ...
    if (
      this.state.startRowIndex !== null &&
      rowIndex >= this.state.startRowIndex &&
      rowIndex <= this.state.endRowIndex &&
      columnIndex >= this.state.startColumnIndex &&
      columnIndex <= this.state.endColumnIndex
    ) {
      //在区间内不处理
    } else {
      //重新设定选择的区间
  ...
  merge() {
    if (
      this.state.startRowIndex !== null &&
      this.state.startRowIndex !== this.endRowIndex &&
      this.state.startColumnIndex !== this.endColumnIndex
    ) {
      let sheet = JSON.parse(JSON.stringify(this.state.sheet));
      ...
      sheet.cells[startRowIndex][startColumnIndex].rowSpan =
        endRowIndex - startRowIndex + 1;
      sheet.cells[startRowIndex][startColumnIndex].colSpan =
  ...
  changeCells(props) {
    ...
    try {
      if (
        this.state.startRowIndex !== undefined &&
        this.state.startRowIndex !== undefined
      ) {
        switch (props) {
          case "addCellDown":
          ...
          case "addCellRight":
            break;
          case "addRow":
            sheet = sheetFunc.insertRow(
              this.state.startRowIndex,
              this.state.sheet
            );
            break;
          case "addColumn":
            sheet = sheetFunc.insertColumn(
              this.state.startColumnIndex,
              this.state.sheet
            );
            break;
          ...
          case "deleteCellRight":
            break;
          case "deleteRow":
            sheet = sheetFunc.deleteRow(
              this.state.startRowIndex,
              this.state.endRowIndex,
              this.state.sheet
            );
            break;
          case "deleteColumn":
            sheet = sheetFunc.deleteColumn(
              this.state.startColumnIndex,
              this.state.endColumnIndex,
              this.state.sheet
            );
            break;
        }
  ...
  render() {
    if (this.props.active) {
      //取选中的单元格中的第一个单元格作为属性
      let selectionCellProps =
        this.state.startRowIndex !== null
          ? this.state.sheet.cells[this.state.startRowIndex][
              this.state.startColumnIndex
            ]
          : {};
      let position = this.getSelectionPosition();
      return (
        <div className="wasabi-excel-sheet" id={this.state.sheetcontainerid}>
          <Tool
            ref={this.tool}
            cellProps={selectionCellProps}
            value={this.state.value}
            onClick={this.onToolClick}
            onChange={this.onChange}
            activeIndex={this.state.activeIndexTool}
            changeToolActive={this.changeToolActive}
          ></Tool>
          <div
            className="wasabi-table-realTable"
            onMouseMove={this.onMouseMove}
            onContextMenu={this.onContextMenu}
          >
            <Table id={this.state.tableid}>
              <SheetColGroup
                headers={this.state.sheet.headers || []}
              ></SheetColGroup>
              <SheetHeader
                headers={this.state.sheet.headers || []}
              ></SheetHeader>
              <SheetBody
                ...
                cells={this.state.sheet.cells || [[]]}
              ></SheetBody>
            </Table>
            <div
              className="wasabi-excel-sheet-selection-layer"
              style={position.layerStyle}
            >
              <div
                key="1"
                className="wasabi-excel-sheet-selection top"
                style={position.topStyle}
              ></div>
              <div
                key="2"
                className="wasabi-excel-sheet-selection left"
                style={position.leftStyle}
              ></div>
              <div
                key="3"
                className="wasabi-excel-sheet-selection right"
                style={position.rightStyle}
              ></div>
              <div
                key="4"
                className="wasabi-excel-sheet-selection bottom"
                style={position.bottomStyle}
              ></div>
              <div
                key="5"
                className="wasabi-excel-sheet-selection dot"
                style={position.dotStyle}
                onMouseDown={this.dotonMouseDown}
              ></div>
            </div>
          </div>
