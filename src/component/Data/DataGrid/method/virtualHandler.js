...
export default {
  ...
  initVirtual() {
    ...
    let visibleHeight =
      Math.ceil(document.getElementById(this.state.containerid).clientHeight) -
      topReduce; //表体可见高度
  ...
  scrollShowVisibleData(startIndex, endIndex) {
    try {
      if (this.virtualConfig.startIndex !== startIndex) {
        ...
        this.table.style.transform = `translate3d(0,${
  ...
  adjustItemHeight() {
    try {
      if (this.virtualConfig && this.virtualConfig.positions) {
        let tr = this.table.children[1]?.children; //取出当前行
  ...
  adjustColumnWidth() {
    try {
      ...
      if (
        document.getElementById(this.state.realTableId) &&
        document.getElementById(this.state.realTableId).children.length === 2 &&
        document.getElementById(this.state.realTableId).children[1].children
          .length
      ) {
        //有数据才调整宽度
