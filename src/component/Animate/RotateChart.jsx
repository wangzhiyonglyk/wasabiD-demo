...
class RotateChart extends React.Component {
      ...
      this.interval = setInterval(() => {
        ...
        //此处处理文字，及重绘制环
        {
          this.timeout = setTimeout(() => {
            this.activeIndex++;
            this.props.oneFinishHandler &&
              this.props.oneFinishHandler(this.activeIndex - 1); //转完一个
    ...
    if (c) {
      ...
      if (arr instanceof Array && arr.length <= 8) {
        ...
        if (this.props.countType == "sum") {
          ...
          for (let i = 0; i < arr.length; i++) {
            model.push({
              ...
              eAngle:
                start +
                ((arr[i] / sum) * sumAngle).toFixed(
                  this.props.decimal ? 2 : 0
                ) *
                  1,
            });
            start +=
              ((arr[i] / sum) * sumAngle).toFixed(this.props.decimal ? 2 : 0) *
                1 +
              2; //下一个开始
          }
        } else {
        ...
        requestAnimationFrame(() => {
          ...
          //画文字
          let text =
            this.props.text instanceof Array && this.props.text.length > 0
              ? this.props.text[activeIndex]
              : this.props.countType == "sum"
              ? ((arr[activeIndex] * 100) / sum).toFixed(
                  this.props.decimal ? 2 : 0
                ) + "%"
              : (arr[activeIndex] + "").replace("%", "") + "%";
  ...
  drawText(ctx, text) {
    ...
    ctx.arc(
      this.width / 2,
      this.height / 2,
      this.props.innerRadius || radius / 2,
      0,
      Math.PI * 2
    );
    ctx.closePath();
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.data &&
      ((this.props.data &&
        nextProps.data.join(",") !== this.props.data.join(",")) ||
        !this.props.data)
    ) {
      //数据有改变
      this.refresh();
    ...
    return (
      ...
      >
        <canvas
          ...
          className="chart"
        ></canvas>
        <div id={this.state.textid} className="text" style={fontStyle}>
          {this.state.text}
        </div>
      </div>
    );
