...
class Left extends React.Component {
    ...
    mouseMoveHandler(event) {
        ...
        if (this.position != null && center) {
            ...
            if (event.clientX - this.oldClientX > 2 || event.clientX - this.oldClientX < -2) {//防止抖动
                target.style.left = ( this.position.leftWidth + event.clientX - this.oldClientX - 3) + "px";
                left.style.width = ( this.position.leftWidth + event.clientX - this.oldClientX) + "px";
                center.style.left = ( this.position.leftWidth + event.clientX - this.oldClientX) + "px";
                center.style.width = "calc(100% - " + (rightWidth +  this.position.leftWidth + event.clientX - this.oldClientX) + "px)";
            }
    ...
    mouseDownHandler(event) {
        ...
        if (dom.isDescendant(document.getElementById(this.state.separatorid), event.target) || event.target.className == "wasabi-separator-left") {
    ...
    render() {
        return <div className={"wasabi-layout-left  layout-panel " + this.props.className} id={this.props.leftid}
            ...
            {this.props.children}
            <div ref="separator" id={this.state.separatorid} className="wasabi-separator-left" style={{ left: this.props.width - 3, top: this.props.top, height: (this.props.reduceHeight ? "calc(100% - " + (this.props.reduceHeight).toString() + "px" : null) }}>
